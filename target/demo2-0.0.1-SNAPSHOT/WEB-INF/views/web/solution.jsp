        ...
        <div>
            ...
            <c:forEach var="item" items="${listSub}" >
                <tr>
                    ...
                    <td>
                    <a href="/web/api/solution/"${item.problemCode}" style="color: #d30000;">${item.problemName}
                    </td>
                    ...
                    <c:if test="${item.status != null}">
                        <c:if test="${item.status == true}">
                            <td><a style="color: #19BE6B;" href="/web/api/submission/${item.id}/edit" />AC</td>
                            <td>${item.time}s</td>
                            <td>${item.memoryUsed}Kb</td>
                        </c:if>
                        <c:if test="${item.status == false}">
                            <c:if test="${item.code == 'CE'}">
                                <td><a style="color: rgb(0, 0, 0)" href="/web/api/submission/${item.id}/edit" />CE</td>
                                <td></td>
                                <td></td>
                            </c:if>
                            <c:if test="${item.code != 'CE'}">
                                <td><a style="color: #FF0000;" href="/web/api/submission/${item.id}/edit" />${item.code}</td>
                                <td>${item.time}s</td>
                                <td>${item.memoryUsed}Kb</td>
                            </c:if>
                        </c:if>
            ...
            if(fileName.split('.').pop() !== 'c' && fileName.split('.').pop() !== 'cpp' && fileName.split('.').pop() !== 'java' && fileName.split('.').pop() !== 'py'){
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        window.location.href = '/web/api/history';
                    },
